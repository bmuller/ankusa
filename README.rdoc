...
== Installation
First, install HBase/Hadoop or Cassandra.  If you're using HBase, make sure the HBase Thrift interface has been started as well.  Then:
 ...
 * Prior to using the Cassandra storage you will need to run the following command from the cassandra-cli.  This should be fixed with a new release candidate for cassandra.
